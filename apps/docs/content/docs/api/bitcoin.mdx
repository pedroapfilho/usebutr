...
Adapter ids come from `slugify("btc", wallet.name)` in
`@usebutr/wallet-standard-shared`; import it from there when authoring an
adapter.
...
[`@usebutr/wallet-standard-shared`](/api/wallet-standard-shared); import them from
