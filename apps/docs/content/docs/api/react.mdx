...
| `initialState`           | `WalletSnapshot`                        | Seeds the store synchronously from a server-read snapshot. See [Hydration](/concepts/hydration#synchronous-seeding-ssr). |
...
| Hook                               | Returns                                                            |
| ---------------------------------- | ------------------------------------------------------------------ |
| `useConnectionStatus()`            | `"idle" \| "connecting" \| "success" \| "error" \| "reconnecting"` |
| `useIsReconnecting(connectorId?)`  | `boolean` (seeded entry awaiting its live adapter)                 |
...
| `useDiscoveredWalletsByPlatform()` | `Map<ChainPlatform, Array<WalletAdapter>>`                         |
| `useConnectedWalletsByPlatform()`  | `Map<ChainPlatform, Array<ConnectedWallet>>`                       |
...
list above repeats a brand once per chain. These return the same wallets bucketed
by platform, in `CHAIN_PLATFORMS` order, with empty platforms omitted, so the map
is directly renderable as well as addressable.

```tsx
import { useDiscoveredWalletsByPlatform } from "@usebutr/react";
...
return [...byPlatform].map(([platform, wallets]) => (
  <section key={platform}>
    <h3>{platform}</h3>
    {wallets.map((w) => (
      <WalletButton key={w.id} adapter={w} />
    ))}
  </section>
));
```

The result is memoised on the source list, so it's stable across re-renders.
For a non-hook context, `groupByPlatform(items, getPlatform)` from
[`@usebutr/core`](/api/core) does the same bucketing on any list.
...
**Source:** `packages/react/src/index.ts`, `hooks/selectors.ts`, `hooks/actions.ts`, `hooks/async-resources.ts`, `hooks/grouped.ts`.
