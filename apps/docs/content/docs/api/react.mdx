...
| `discovery` | `WalletSource` | Auto-discovery source. Omit for a fully-manual provider (no protocol code enters the bundle). |
...
```tsx
import { useDiscoveredWallets } from "@butr/react";
...
mounted. Sourced from `@butr/react`.
