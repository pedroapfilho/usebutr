...
graph TD
  ...
  subgraph integration["Integration"]
    signer["getSigner(): escape hatch<br>EIP-1193 / WalletStandardWallet / PolkadotSignerHandle"]
  end
...
| Polkadot | `PolkadotSignerHandle` (`{ extensionName, address }`): bridge to `polkadot-api` via `pjs-signer`. |
...
Adding a sixth `ChainPlatform` is a fixed-shape change touching the same
...
The five current platforms (EVM, SVM, Sui, Bitcoin, Polkadot) each ship
the same shape. The Ledger dispatch's `default:` branch is a true `never`
