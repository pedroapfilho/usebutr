---
title: Architecture
description: End-to-end runtime — how discovery, adapters, the store, the pool, and React hooks fit together across four platforms.
---
...
- **Wallet Standard** for SVM, Sui, and Bitcoin — wallets register through
  the global `@wallet-standard/app` registry.
...
`@usebutr/wallets` ships `autoDiscovery()` which composes all four platforms.
...
pool holds every connection across all four platforms simultaneously. The
...
That's the whole library. Six layers, one adapter contract, four platforms.
