...
(`WalletBase`, or one of its per-platform extensions such as `SvmWallet`). The
split is documentary: it makes the orchestration / capability seam explicit.
...
## WalletBase: what your app calls
...
chain travels _inside_ the account. Switching chains re-wraps the address with
new chain data.
