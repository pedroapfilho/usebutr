...
```tsx
import { useConnectionError } from "@butr/react";
...
`mapConnectionError(raw)` (exported from `@butr/core`) does the normalisation.
It recognises:
...
<Callout type="warn">
  Normalisation is best-effort. A custom connector that throws an
  unrecognised shape lands in `Unknown`. butr does no retrying — you decide
  whether to retry based on `kind`.
</Callout>
