...
overwriting. Both eager- and late-restored entries survive. In manual discovery
setups you trigger this yourself after discovering an adapter:

```ts
void store.getState()._tryRestoreFromPending(adapter.id);
```

`AutoWalletManagerProvider` does this for you; you only write it in custom
discovery wiring (see the [Next.js guide](/docs/frameworks/nextjs)).
