...
<Callout type="warn">
  Because writes are unobserved, a failed write is invisible unless you ask. Set
  `onStorageError` to surface them. `context` is a short string naming the
  failed write (e.g. `"failed to persist pool"`). The default behaviour when no
  callback is set is `console.warn`.
</Callout>
