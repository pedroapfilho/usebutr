...
- **Wallet Standard (SVM, Sui, Bitcoin, Polkadot)**: `@usebutr/svm`,
  `@usebutr/sui`, `@usebutr/bitcoin`, and `@usebutr/polkadot` lazily import
  `@wallet-standard/app` and use `getWallets()`: `get()` for wallets already
  present, `on('register')` for ones that load later, and `on('unregister')`
  to drop a wallet whose extension is disabled or removed mid-session.
- **EVM**: `@usebutr/evm` adds the `eip6963:announceProvider` listener _before_
...
(unauthorized / disconnected) map to a `ConnectionError` so consumers branch
on `kind` instead of regexing messages. See [errors](/concepts/errors).

Feature presence drives [capabilities](/concepts/capabilities): a Wallet
Standard adapter defines `signMessage`, `signTransaction`, `signIn`, and
`sendTx` only when the wallet advertises the underlying feature, so an adapter
never claims more than the wallet supports.

## Deliberate edges

These are intentional boundaries, not gaps:

- **No RPC.** Wallet Standard adapters have no `getBalance` or
  `getTransactionReceipt`: butr hands you a signer and stays out of the RPC
  business. Wrap your own client (`@solana/kit`, `@mysten/sui`, …). EVM
  adapters read both through the wallet's own EIP-1193 provider.
- **`solana:signTransaction` is sign-only.** butr returns the signed bytes;
  you broadcast them, because butr ships no RPC. `signTransaction` exists only
  when the wallet advertises the feature.
- **No wallet-side account switch.** Neither spec has a "use address X" RPC,
  so the user picks the active account in the wallet's own UI and butr mirrors
  it via the change event. `setAccount(id, account)` changes which exposed
  account butr treats as active; pass it as `{ account }` to sign with it.
- **Features are read once.** Discovery builds an adapter from the features
  the wallet advertises at registration. A wallet that swaps its feature set
  at runtime keeps the adapter's original methods until the page reloads.
...
<Callout type="info">
  **Source:** `packages/svm/src` (Wallet Standard), `packages/evm/src` (EIP-6963 / EIP-1193),
  `packages/core/src/store/wallet-manager.ts` (silent reconnect),
  `packages/core/src/types/errors.ts` (error mapping). Behaviour is exercised by the package test
  suites and the runnable `apps/demo-with-*` demos.
</Callout>
