...
`AutoWalletManagerProvider` does this when SVM discovery is enabled (default).

<Callout type="info">
  `@butr/svm` lazily imports `@wallet-standard/app` (an optional peer
  dependency). That asynchronous warmup is one reason restored SVM wallets can
  land in `pendingIds` for a moment — see
...
<Callout type="info">
  **Source:** `packages/svm/src`. Used by all `apps/demo-with-solana-*` demos
  and (via `@butr/wallets`) `apps/demo-vite`.
</Callout>
