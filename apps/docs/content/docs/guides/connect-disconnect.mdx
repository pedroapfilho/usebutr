...
(In a [manual EVM-only setup](/docs/guides/provider-setup), `useDiscoveredWallets`
comes from your own provider module instead of `@butr/wallets`.)
...
{error ? <p>{error.kind} — {error.message}</p> : null}
```
...
<Callout type="info">
  **Source:** `apps/demo-vite/src/app.tsx` and `apps/demo-next/src/app/page.tsx`
  in the [butr repository](https://github.com/pedroapfilho/butr/tree/main/apps).
