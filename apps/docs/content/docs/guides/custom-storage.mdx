...
<Callout type="warn">
  ...
  store and accepts that session entries outlive the session. That's harmless here, since butr's
  session storage only holds the active-connector id, which is overwritten on the next connect. If
  you need strict session semantics, namespace session keys and sweep them on startup.
</Callout>
