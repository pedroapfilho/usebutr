...
```tsx
import { useActiveWallet, useSetActiveConnector } from "@butr/react";
...
{isActive ? null : (
  <button onClick={() => setActive(wallet.connector.id)}>Make active</button>
)}
```

See [pool, selection, active](/docs/concepts/pool-selection-active) for why
these are separate.
...
<Callout type="info">
  EVM-only manual setups don't bundle `@butr/wallets`. Use `EVM_CHAINS_LIST`
  from `@butr/evm` instead — `demo-next` does exactly this.
</Callout>

<Callout type="info">
  **Source:** `ChainPicker` in `apps/demo-vite/src/app.tsx` (uses
  `CHAINS_BY_PLATFORM`) and `apps/demo-next/src/app/page.tsx` (uses
  `EVM_CHAINS_LIST`).
</Callout>
