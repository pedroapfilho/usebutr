...
Use `autoDiscovery()` from `@usebutr/wallets`. It discovers EVM (EIP-6963),
SVM, Sui, and Bitcoin (Wallet Standard), and Polkadot (injectedWeb3) wallets
and persists state to `localStorage`. Hoist the source to module scope so it
is created once.
...
<Callout type="info">
  **Source:** `apps/demo-vite/src/wallet-provider.tsx` (auto, multi-chain) in the [butr
