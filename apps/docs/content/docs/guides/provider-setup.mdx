...
This is the pattern used by `demo-next` and `demo-tanstack-start`; both
re-export `useDiscoveredWallets` from `@butr/react` so consuming files import it
from the local `wallet-provider` module.
