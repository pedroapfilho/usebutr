...
1. If the wallet is an SVM wallet with `connector.signIn` (it advertises
   `solana:signIn`) and `preferSignMessage` is not `true`, fetch a nonce and
   take the wallet's SIWS path.
2. Otherwise, if `connector.signMessage` is absent, throw
   `SignInUnsupportedError` before fetching a nonce or touching the wallet.
3. Fetch a nonce for the explicit account argument, or `wallet.account`.
4. Build the message and call `signMessage` with its UTF-8 bytes and
   `{ account }`.
...
has occurred. Offer a signing-capable wallet in that case, or hide the sign-in
button up front: a wallet can sign in when it has `connector.signMessage`, or
is an SVM wallet with `connector.signIn`.

A wallet that rejects the prompt rejects `signIn()` with its own error; pass
it through `toConnectionError` to branch on `kind` (see
