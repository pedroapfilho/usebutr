...
<Callout type="warn">
  Verify the signature against the returned `signedMessage`, **not** your input
  bytes. Solana Wallet Standard wallets may prefix or re-encode the message
  internally. EVM wallets echo the input, so there it's the same bytes — but
  writing verification against `signedMessage` works for both.
</Callout>
...
<Callout type="info">
  **Source:** `AccountRow` in `apps/demo-vite/src/app.tsx`; `handleSign` in
  `apps/demo-with-solana-kit/src/app.tsx` and
