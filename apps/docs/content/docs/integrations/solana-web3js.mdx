...
<Callout type="warn">
  Feature-detect every time (`if (!feature) throw`). Wallets advertise the
  Wallet Standard features unevenly; gate UI on
  `wallet.connector.capabilities.sendTransaction` /
  `.signMessage`.
</Callout>

<Callout type="info">
  **Source:** `apps/demo-with-solana-web3js/src/app.tsx` in the
  [butr repository](https://github.com/pedroapfilho/butr/tree/main/apps/demo-with-solana-web3js).
