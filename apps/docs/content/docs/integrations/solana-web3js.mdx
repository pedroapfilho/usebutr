...
<Callout type="warn">
  **Legacy.** `@solana/web3.js` v1 is in maintenance mode. For new apps, prefer
  [framework-kit](/integrations/solana-framework-kit) (the recommended modern
  stack), [gill](/integrations/gill) (ergonomic, for most apps), or
  [`@solana/kit`](/integrations/solana-kit) (low-level). This page is kept for
  apps still on v1.
</Callout>
