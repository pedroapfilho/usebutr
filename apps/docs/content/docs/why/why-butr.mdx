...
`getSigner()` returns the wallet's own provider, tagged by transport, so you
narrow it with `signer.kind` instead of casting. You bridge it into viem, wagmi,
...
`@usebutr/core` is the wallet manager: discovery, hydration, and persistence
behind `createWalletManager`, usable from any TypeScript runtime.
`@usebutr/react` is the binding, so the same connection model works across
Vite, Next.js, TanStack Start, and Expo without rewrites.
